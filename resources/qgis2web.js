...
map.getView().fit([-7017360.025642, -3188021.947301, -5569444.168542, -2503764.937414], map.getSize());
...
function onSingleClickFeatures(evt) {
    ...
    // 1. Clear previous dimensions immediately
    removeAllLineLabels(); 
    ...
    map.forEachFeatureAtPixel(pixel, function(feature, layer) {
        // --- START OF LABEL DRAWING LOGIC ---
        // Check for both 'COMBINADO' and 'Combinado' to be safe
        var layerTitle = layer ? layer.get('popuplayertitle') : '';
        if (layerTitle === 'COMBINADO' || layerTitle === 'Combinado') {

            if (feature && (feature.getGeometry().getType() === 'MultiPolygon' || feature.getGeometry().getType() === 'Polygon')) {

                var geometry = feature.getGeometry();
                // Get projected coordinates (EPSG:3857)
                ...
                for (var i = 0; i < coords_proj.length - 1; i++) {
                    ...
                    // Transform for accurate meter calculation
        ...
        // Continue with original popup logic
        if (layer && feature instanceof ol.Feature && (layer.get("interactive") || layer.get("interactive") === undefined)) {
            var doPopup = false;
...
var searchLayer = new SearchLayer({
    layer: lyr_FRACCION_0,
    colName: 'FRACCIONES_Código - Nombre de Fracción',
    zoom: 10,
    collapsed: true,
    map: map,
    maxResults: 10,
